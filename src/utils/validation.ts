...
export const currentCostsSchema = z.object({
  ...
  inboxesPerDomain: z.number().min(1, 'Inboxes per domain must be at least 1'),
  costPerInbox: z.number().min(0, 'Cost per inbox must be 0 or greater'),
...
export const ourOfferSchema = z.object({
  ...
  inboxesPerDomain: z.number().min(1, 'Inboxes per domain must be at least 1'),
  sendsPerInbox: z.number().min(1, 'Sends per inbox must be at least 1'),
  costPerInbox: z.number().min(0, 'Cost per inbox must be 0 or greater'),
